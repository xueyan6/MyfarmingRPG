            ...
            if (npcIsMoving == false) // 如果NPC不在移动状态
            {
                ...
                if (npcPath.npcMovementStepStack.Count > 0) // 如果路径栈中有移动步骤
                {
                    ...
                    if (npcCurrentScene.ToString() == SceneManager.GetActiveScene().name)
                    {
                        ...
                        npcMovementStep = npcPath.npcMovementStepStack.Pop(); // 弹出移动步骤

                        npcNextGridPosition = (Vector3Int)npcMovementStep.gridCoordinate; // 获取下一个网格坐标

                        TimeSpan npcMovementStepTime = new TimeSpan(npcMovementStep.hour, npcMovementStep.minute, npcMovementStep.second); // 创建移动步骤时间

                        MoveToGridPosition(npcNextGridPosition, npcMovementStepTime, TimeManager.Instance.GetGameTime()); // 移动到网格位置
                    }
                }
                // 如果没有更多NPC移动步骤
                else
